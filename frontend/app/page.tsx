...
import { BREAKS, type BreakId } from "@/lib/surf-data"
      ...
      <section style={{ position: "relative", height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 20, maxWidth: 900, padding: "0 32px", textAlign: "center" }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.70)", fontSize: 19, fontWeight: 300, lineHeight: 1.65, maxWidth: 520, margin: "0 auto 52px" }}>
            Live buoy, wind, and tide data for La Jolla Shores, Blacks Beach, and Pacific Beach Point — powered by NDBC, NOAA, and a machine learning model trained on 3 years of surf data.
          </p>
      ...
      <div>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "80px 32px 64px", textAlign: "center" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
            {(Object.keys(BREAKS) as BreakId[]).map((id) => (
              <Link
                key={id}
                href={`/dashboard?break=${id}`}
                className="group"
                style={{ textAlign: "left", borderRadius: 10, padding: "18px 20px", background: "#0d1b29", border: `1px solid ${NAV_BORDER}`, borderTop: `3px solid ${BRAND}`, textDecoration: "none", display: "block" }}
              >
                ...
                <div style={{ color: "#fff", fontSize: 16, fontWeight: 500, lineHeight: 1.4, marginBottom: 5 }}>
                  {BREAKS[id].loc}
                </div>
